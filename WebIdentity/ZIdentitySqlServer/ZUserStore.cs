...
using System.Security.Claims; // IUserClaimStore
using ZLib.DB;
...
namespace ZIdentitySqlServer
{
    ...
    /// </summary>
    public class ZUserStore<TUser> : IUserLoginStore<TUser>,
        ...
        where TUser : ZUser
    {
        private readonly ZUserTable<TUser> userTable;
        private readonly ZRoleTable roleTable;
        private readonly ZUserRolesTable userRolesTable;
        private readonly ZUserClaimsTable userClaimsTable;
        private readonly ZUserLoginsTable userLoginsTable;
        ...
        //public ZUserStore(string sConnectionString)
        //{
        //    new ZUserStore<TUser>(new ZIdentityDB(sConnectionString));
        //}

        public ZUserStore(ZSqlClient db1)
        {
            MyDB1 = db1;
            userTable = new ZUserTable<TUser>(db1);
            roleTable = new ZRoleTable(db1);
            userRolesTable = new ZUserRolesTable(db1);
            userClaimsTable = new ZUserClaimsTable(db1);
            userLoginsTable = new ZUserLoginsTable(db1);
        }
        ...
        public Task<TUser> FindByIdAsync(string userId)
        {
            ...
            if (userTable.GetUserById(userId) is TUser result)
            {
        ...
        public Task<TUser> FindByNameAsync(string userName)
        {
            ...
            if (userTable.GetUserByName(userName) is List<TUser> result && result.Count == 1)
            {
        ...
        public Task<TUser> FindAsync(UserLoginInfo login)
        {
            ...
            if (userId != null)
            {
                ...
                if (userTable.GetUserById(userId) is TUser user)
                {
        ...
        public Task<TUser> FindByEmailAsync(string email)
        {
            ...
            if (userTable.GetUserByEmail(email) is TUser result)
            {
                return Task.FromResult<TUser>(result);
            }
